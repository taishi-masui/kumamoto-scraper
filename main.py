...
def main():
    ...
    targets = [
        {
            "name": "熊本県", "code": "0100",
            "n_types": ["1002011", "1002012"], # 一般, 通常指名
            "g_list": ["0100010", "0100130", "0100050"], # 土木, 舗装, とび
            "h_tanto": "25" # 阿蘇
        },
        {
            "name": "南小国町", "code": "0423",
        ...
        },
        {
            "name": "小国町", "code": "0424",
            "n_types": [""], # 指定なし
            "g_list": ["0100010", "0100130", "0100050"], # 県と同じ3業種
            "h_tanto": ""
        }
    ]
                ...
                for n_type in t["n_types"]:
                    for g_val in t["g_list"]:
                            ...
                            for f in page.frames:
                                try:
                                    if f.locator('select[name="GYOSYU_TYPE"]').count() > 0:
                                        f.locator('select[name="GYOSYU_TYPE"]').select_option("00") # 工事固定
                        ...
                        if detail_f:
                            ...
                            # 一覧の2列目（施行番号/案件番号）を含む基本情報を取得
                            base_info = [t['name']] + [c.inner_text().strip() for c in target_f.locator("#tBody tr").nth(0).locator("td").all()][0:4]
